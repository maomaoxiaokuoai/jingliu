...
data class LensPlan(val tier:OpticalTier,val depth:Float,val bevel:Float,val dispersion:Float,val saturation:Float)
object LensPolicy {
    fun plan(s:UiSettings,role:GlassRole,sdk:Int,hardware:Boolean,enabled:Boolean,
             minSide:Float,density:Float):LensPlan {
        if(!enabled || !hardware || sdk<31 || s.text("material")=="solid" || s.enabled("reduceTransparency") ||
            ...
            return LensPlan(OpticalTier.SOLID,0f,1f,0f,1f)
        ...
        if(!lens)return LensPlan(OpticalTier.BLUR,0f,1f,0f,saturation)
        ...
        return LensPlan(OpticalTier.LENS,depth,bevel,(s.number("dispersion")/100f).coerceIn(0f,.12f),saturation)
    }
}
