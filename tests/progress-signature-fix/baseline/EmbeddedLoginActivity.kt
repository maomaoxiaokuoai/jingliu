            ...
            setContent {
                MaterialTheme { Surface(Modifier.fillMaxSize()) {
                    Column(Modifier.safeDrawingPadding().padding(24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                        ...
                        Button(onClick = { finish() }) { Text("返回账号页") }
                    }
                } }
            }
            return
    ...
    @Composable private fun BrowserScreen() {
        ...
        CompositionLocalProvider(LocalUiSettings provides settings) {
            LumaTheme(settings.appearance()) {
                ...
                Scaffold(
                    topBar = {
                        TopAppBar(
                            ...
                            navigationIcon = { TextButton(onClick = { closeBrowser() }) { Text("关闭") } },
                            actions = {
                                TextButton(onClick = { showOptions = !showOptions }) { Text("显示") }
                                TextButton(onClick = { browser?.reload() }, enabled = ready && !busy) { Text("刷新") }
                            }
                        )
                    },
                    bottomBar = {
                        Column(Modifier.navigationBarsPadding().imePadding().padding(horizontal = 12.dp, vertical = 8.dp),
                            ...
                            if (showOptions) {
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    FilterChip(selected = desktop, onClick = { changeMode(true) }, label = { Text("电脑版") })
                                    FilterChip(selected = !desktop, onClick = { changeMode(false) }, label = { Text("手机版") })
                                    if (platform == Platform.KUAISHOU) FilterChip(selected = lightweight,
                            ...
                            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                ...
                                Button(onClick = { lifecycleScope.launch { capture(true) } },
                ...
                if (confirmScreenshot) AlertDialog(onDismissRequest = { confirmScreenshot = false },
                    title = { Text("允许本次截图？") }, text = { Text("页面可能包含账号或二维码，请勿分享。") },
                    confirmButton = { TextButton(onClick = { confirmScreenshot = false; screenshots = true;
                        window.clearFlags(WindowManager.LayoutParams.FLAG_SECURE) }) { Text("允许") } },
                    dismissButton = { TextButton(onClick = { confirmScreenshot = false }) { Text("取消") } })
            }
        }
    }
    ...
    private fun createBrowser(): WebView {
        ...
        view.webViewClient = object : WebViewClient() {
            ...
            override fun onPageStarted(view: WebView, url: String, favicon: Bitmap?) {
                pageVisible = false; progress = 0; openAt = SystemClock.elapsedRealtime()
        ...
        view.webChromeClient = object : WebChromeClient() {
            override fun onProgressChanged(view: WebView, value: Int) { progress = value.coerceIn(0, 100) }
        }
